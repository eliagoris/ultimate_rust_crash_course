...
fn main() {
    ...
    fn eat(string: String) -> bool {
        if string.starts_with("b") && string.contains("a") { true } else { false }
    }
    ...
    fn bedazzle(string: &mut String) {
        // can't assign a new value before deferencing
        // because you can't *completely* change the value of a reference (you can only insert),
        // can only change a value in memory

        // this will give error: mismatched types expected `&mut String`, found struct `String`
        // string = String::from("sparkly");

        // this will work because it is not replacing the value
        string.insert_str(0, "sparkly");

        // dereference before changing the value
        // this will change the value of the initial variable that owns the value in memory.
        *string = String::from("sparkly");
    }
}